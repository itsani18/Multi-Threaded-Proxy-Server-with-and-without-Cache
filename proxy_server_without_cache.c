...
/* Windows (Visual Studio / MinGW) socket headers — prevents the language server
   from trying to open POSIX headers like <sys/socket.h> on Windows */
#include <winsock2.h>
#include <ws2tcpip.h>
#include <io.h>

/* Note: the rest of this program uses POSIX APIs (pthread, semaphores, unistd, etc.)
   which are not available natively on Windows; this conditional only avoids the
   specific include error for sys/socket.h when editing on Windows. Building on
   Windows may require additional porting (or using WSL/Cygwin/MinGW POSIX layers). */
